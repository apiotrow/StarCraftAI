...
void ExampleAIModule::onFrame()
{
	...
	for ( Unitset::iterator u = myUnits.begin(); u != myUnits.end(); ++u )
	{
		...
		//if unit is vulture
		else if ( u->getType() == UnitTypes::Enum::Terran_Vulture)
		{
			...
			if((id1 != 999 && id1 == u->getID())){
					closestTarget = 
						u->getClosestUnit(GetType == UnitTypes::Enum::Zerg_Drone);
					if (closestTarget == 00000000){
						closestTarget = u->getClosestUnit(GetType == UnitTypes::Enum::Zerg_Hatchery);
					}
					if (closestTarget == 00000000){
						closestTarget = u->getClosestUnit(GetType == UnitTypes::Enum::Zerg_Creep_Colony);
					}
					if (closestTarget != 00000000){
						Unit testTarget =
							u->getClosestUnit(GetType == UnitTypes::Enum::Zerg_Zergling);
						if (u->getDistance(closestTarget) > u->getDistance(testTarget)) closestTarget = testTarget;
					}
					...
					closestTarget = 
						u->getClosestUnit(GetType == UnitTypes::Enum::Zerg_Zergling);
					if (closestTarget == 00000000){
						closestTarget = u->getClosestUnit(GetType == UnitTypes::Enum::Zerg_Drone);
					}
					if (closestTarget == 00000000){
						closestTarget = u->getClosestUnit(GetType == UnitTypes::Enum::Zerg_Hatchery);
					}
			...
			Position pos_u;
			Position pos_e;
			pos_e = closestTarget->getPosition(); //enemy position
			pos_u = u->getPosition(); //our unit position
			if (closestTarget != nullptr){
				if(distToTarget > 165){
					u->attack(Broodwar->getUnit(closestTarget->getID()),false);
				}
				else if (!closestTarget->getType().isBuilding()){
					/*
					Unitset self = Broodwar->getUnitsInRadius(pos_u, 75);
					int selfcount = std::distance(self.begin(), self.end());
					//int oldspidercount = u->getSpiderMineCount();
					//int newspidercount = u->getSpiderMineCount();
					if(selfcount > 2){
						Broodwar << "surrounded" << std::endl;
						//u->useTech(TechTypes::Enum::Spider_Mines, pos_u);
						/*
						while (newspidercount == oldspidercount){
							newspidercount = u->getSpiderMineCount();
						}
